...
public class Mat {
	...
	public Mat setIdentity() {
		return setIdentity(1);
	}

	public Mat setIdentity(double scale) {
	...
	public Mat add(Mat other) {
		if (this.getRows() != other.getRows() || this.getColums() != other.getColums())
			throw new IllegalArgumentException("can't add up " + this.getRows() + "x" + this.getColums() + "- and " + other.getRows() + "x" + other.getColums() + "-Matrix");
	...
	public Vec mul(Vec other) {
		...
		if (mat_col != vec_row)
			throw new IllegalArgumentException("can't multiply" + mat_row + "x" + mat_col + "-Matrix with " + vec_row + "D-Vector.");
...
//		if(tmpBuffer==null || tmpBuffer.capacity()<mat.length*mat[0].length){
//			tmpBuffer = ByteBuffer.allocateDirect(mat.length*mat[0].length*8).order(ByteOrder.nativeOrder()).asDoubleBuffer();
//		}
		buff.position(0);
		int rows = this.getRows();
		int columns = this.getColums();
		for (int r = 0; r < 4; r++) {
			for (int c = 0; c < 4; c++) {
				buff.put(c < columns && r < rows ? mat[r][c] : r == c ? 1 : 0);
			}
		}
		buff.position(0);
	}
	...
	public static IVec solve(Mat a, IVec b, double omega, double epsilon) {
		int n;
		if (a.getColums() != (n = a.getRows()))
			throw new IllegalArgumentException("expected a square-matrix");
		if (b.getDimensionCount() != n)
			throw new IllegalArgumentException("expeted the given vector to be of length n");
		Vec xNew, xOld;
		xNew = new Vec(n);
		do {
			xOld = xNew;
			xNew = new Vec(n);
			for (int i = 0; i < n; i++) {
				double finalResult = b.get(i);
				double tmp = 0;
				for (int j = 0; j < i; j++) {
					tmp += a.getValue(i, j) * xNew.get(j);
				}
				finalResult -= tmp;
				tmp = 0;
				for (int j = i + 1; j < n; j++) {
					tmp += a.getValue(i, j) * xOld.get(j);
				}
				finalResult -= tmp;
				xNew.set(i, (1 - omega) * xOld.get(i) + omega / a.getValue(i, i) * finalResult);
			}
	...
	public void readMatrixFromConsole(Scanner s) {
		for (int i = 0; i < this.getRows(); i++) {
			...
			if (input.length != this.getColums()) {
				throw new IllegalArgumentException("expected " + this.getColums() + " elements but got " + input.length + " elements.");
			}
